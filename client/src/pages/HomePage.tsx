  ...
  return (
    <div className="grid gap-4">
      <section className="rounded-md border border-line bg-white p-4 shadow-soft">
        <div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-center">
          <div>
            ...
            <p className="text-sm text-slate-500">{filtered.length} channels in view</p>
          </div>
        ...
        <div className="mt-4 flex gap-2 overflow-x-auto pb-1 scrollbar-none">
          <button className={`flex min-h-10 shrink-0 items-center gap-2 rounded-md border px-3 text-sm font-medium ${!activeGroup ? "border-accent bg-accent text-white" : "border-line bg-white"}`} onClick={() => setActiveGroup("")}>
            <Filter size={16} /> All
          </button>
          <button className={`flex min-h-10 shrink-0 items-center gap-2 rounded-md border px-3 text-sm font-medium ${favoritesOnly ? "border-berry bg-berry text-white" : "border-line bg-white"}`} onClick={() => setFavoritesOnly(!favoritesOnly)}>
            <Star size={16} /> Favorites
          </button>
          {groups.map((group) => (
            <button key={group} className={`min-h-10 shrink-0 rounded-md border px-3 text-sm font-medium ${activeGroup === group ? "border-ink bg-ink text-white" : "border-line bg-white"}`} onClick={() => setActiveGroup(group)}>
              {group}
            </button>
          ))}
        </div>
          ...
          <div className="mt-3 grid gap-2 md:grid-cols-2">
            {search.channels.map((channel) => (
              <Link className="flex items-center gap-3 rounded-md border border-line p-3 hover:border-accent" key={`c-${channel.id}`} to={`/channel/${channel.id}`}>
                ...
                <div className="min-w-0">
                  ...
                  <div className="truncate text-sm text-slate-500">{channel.group_title}</div>
                </div>
              </Link>
            ))}
            {search.programs.map((program) => (
              <Link className="rounded-md border border-line p-3 hover:border-accent" key={`p-${program.id}`} to={`/channel/${program.channel_id}`}>
                ...
                <div className="truncate text-sm text-slate-500">{program.channel_name}</div>
              </Link>
            ))}
          </div>
        </section>
      ...
      <section className="grid gap-3">
        {filtered.map((item) => (
          <article key={item.channel_id} className="rounded-md border border-line bg-white p-3 shadow-soft">
            <div className="grid grid-cols-[auto_1fr_auto] gap-3">
              ...
              <Link to={`/channel/${item.channel_id}`} className="min-w-0">
                <div className="truncate text-sm font-semibold text-slate-500">{item.display_name}</div>
                <h2 className="truncate text-lg font-bold">{item.title ?? "No guide data"}</h2>
                <p className="line-clamp-2 text-sm text-slate-600">{item.description ?? item.group_title}</p>
