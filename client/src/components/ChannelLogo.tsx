...
export function ChannelLogo({ src, name, size = "md" }: { src?: string; name: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={`${sizeClass} grid shrink-0 place-items-center overflow-hidden rounded-md border border-line bg-panel`}>
      {src ? (
        <img src={src} alt="" className="max-h-full max-w-full object-contain p-1" loading="lazy" />
      ) : (
        <Tv size={size === "lg" ? 28 : 20} className="text-ink/45" aria-label={name} />
      )}
    </div>
  );
}
